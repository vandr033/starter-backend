...
export async function getFloor(companyId: number, atValue?: string | Date, shiftId?: number): Promise<Result> {
  ...
  const [areas, tables, reservations, shift, waitlistRows] = await Promise.all([
    ...
    prisma.restaurantWaitlist.groupBy({ by: ['status'], where: { company_id: companyId, status: { in: ['WAITING', 'NOTIFIED', 'ARRIVED'] } }, _count: { _all: true } }),
  ]);
...
async function assertCombination(client: Tx, companyId: number, combinationId: number, reservation: any) {
  ...
  if (!combination || combination.tables.length < 2 || !combination.dining_area || !combination.dining_area.is_active || combination.dining_area.company_id !== companyId || combination.tables.some((item: any) => item.company_id !== companyId || item.table.company_id !== companyId || !item.table.is_active || !item.table.dining_area.is_active) || combinationCapacity(combination) < reservation.party_size) throw Object.assign(new Error('La combinación no está disponible o no tiene capacidad suficiente.'), { status: 409 });
...
export async function relocate(companyId: number, reservationId: number, actorUserId: string, input: { table_id?: number; combination_id?: number; reason?: string | null; initiated_from_at_risk?: boolean }): Promise<Result> {
  try {
    const moved = await prisma.$transaction(async (tx) => {
      ...
      const changed = await tx.restaurantReservation.updateMany({ where: { id: reservationId, company_id: companyId, table_id: reservation.table_id, combination_id: reservation.combination_id }, data: { table_id: combination ? null : table!.id, combination_id: combination?.id || null } });
      if (changed.count !== 1) throw Object.assign(new Error('La reserva cambió mientras se reubicaba.'), { status: 409 });
      const updated = await tx.restaurantReservation.findFirst({ where: { id: reservationId, company_id: companyId } });
      if (!updated) throw Object.assign(new Error('No encontramos la reserva reubicada.'), { status: 404 });
...
export async function updateTableStatus(companyId: number, tableId: number, actorUserId: string, status: RestaurantTableOperationalStatus, blockedReason?: string | null): Promise<Result> {
  try { const state = await prisma.$transaction(async (tx) => {
    await tx.$queryRaw`SELECT id FROM restaurant_table WHERE company_id = ${companyId} AND id = ${tableId} FOR UPDATE`;
    const table = await tx.restaurantTable.findFirst({ where: { id: tableId, company_id: companyId, is_active: true } }); if (!table) throw Object.assign(new Error('No encontramos la mesa.'), { status: 404 });
    const existing = await tx.restaurantTableOperationalState.findUnique({ where: { table_id: tableId } }); if (existing && existing.company_id !== companyId) throw Object.assign(new Error('El estado de la mesa no pertenece a la empresa activa.'), { status: 409 });
    const current = existing?.status || 'AVAILABLE'; if (!transitions[current].includes(status)) throw Object.assign(new Error(`No podés pasar la mesa de ${current} a ${status}.`), { status: 409 });
    const now = new Date(); const updated = await tx.restaurantTableOperationalState.upsert({ where: { table_id: tableId }, create: { company_id: companyId, table_id: tableId, status, status_since: now, occupied_at: ['SEATED', 'BILL_REQUESTED'].includes(status) ? now : null, bill_requested_at: status === 'BILL_REQUESTED' ? now : null, cleaning_started_at: status === 'CLEANING' ? now : null, blocked_reason: status === 'BLOCKED' ? blockedReason || null : null, updated_by_user_id: actorUserId }, update: { company_id: companyId, status, status_since: now, occupied_at: ['SEATED', 'BILL_REQUESTED'].includes(status) ? (existing?.occupied_at || now) : null, bill_requested_at: status === 'BILL_REQUESTED' ? now : null, cleaning_started_at: status === 'CLEANING' ? now : null, blocked_reason: status === 'BLOCKED' ? blockedReason || null : null, updated_by_user_id: actorUserId } });
    await tx.restaurantAuditLog.create({ data: { company_id: companyId, actor_user_id: actorUserId, event: 'TABLE_STATUS_CHANGED', target_type: 'RESTAURANT_TABLE', target_id: String(tableId), table_id: tableId, previous_values: { status: current, blocked_reason: existing?.blocked_reason || null }, new_values: { status, blocked_reason: status === 'BLOCKED' ? blockedReason || null : null } } }); return updated;
  }, { isolationLevel: Prisma.TransactionIsolationLevel.Serializable }); return ok(state, 'Estado de mesa actualizado.'); } catch (error: any) { return fail(error?.status || (error?.code === 'P2034' ? 409 : 500), error?.message || 'No pudimos actualizar el estado de la mesa.'); }
}
...
export async function createCombination(companyId: number, actorUserId: string, input: { name: string; dining_area_id?: number | null; table_ids: number[]; is_active?: boolean }): Promise<Result> {
  try { const combo = await prisma.$transaction(async (tx) => {
    if (new Set(input.table_ids).size !== input.table_ids.length || input.table_ids.length < 2) throw Object.assign(new Error('Una combinación debe incluir al menos dos mesas distintas.'), { status: 400 });
    await tx.$queryRaw`SELECT id FROM restaurant_table WHERE company_id = ${companyId} AND id IN (${Prisma.join(input.table_ids)}) FOR UPDATE`;
    const tables = await tx.restaurantTable.findMany({ where: { company_id: companyId, id: { in: input.table_ids }, is_active: true, dining_area: { is_active: true } }, include: { dining_area: true } });
    if (tables.length !== input.table_ids.length) throw Object.assign(new Error('Todas las mesas deben pertenecer a la empresa y estar activas.'), { status: 400 });
    const areaIds = new Set(tables.map((table) => table.dining_area_id)); if (areaIds.size > 1) throw Object.assign(new Error('Las mesas combinadas deben pertenecer a la misma área.'), { status: 400 });
    if (input.dining_area_id && (!areaIds.has(input.dining_area_id) || !await tx.restaurantDiningArea.findFirst({ where: { id: input.dining_area_id, company_id: companyId, is_active: true }, select: { id: true } }))) throw Object.assign(new Error('El área seleccionada no coincide con las mesas o está inactiva.'), { status: 400 });
    const overlapping = await tx.restaurantTableCombinationTable.findFirst({ where: { company_id: companyId, table_id: { in: input.table_ids }, combination: { is_active: true } } }); if (overlapping) throw Object.assign(new Error('Una de las mesas ya pertenece a otra combinación activa.'), { status: 409 });
    const created = await tx.restaurantTableCombination.create({ data: { company_id: companyId, name: input.name.trim(), dining_area_id: input.dining_area_id || tables[0].dining_area_id, is_active: input.is_active ?? true, created_by_user_id: actorUserId, tables: { create: input.table_ids.map((table_id) => ({ company_id: companyId, table_id })) } }, include: { tables: { include: { table: true } } } });
    await tx.restaurantAuditLog.create({ data: { company_id: companyId, actor_user_id: actorUserId, event: 'TABLE_COMBINATION_CREATED', combination_id: created.id, metadata: { table_ids: input.table_ids } } }); return created;
  }, { isolationLevel: Prisma.TransactionIsolationLevel.Serializable }); return ok(combo, 'Combinación creada.', 201); } catch (error: any) { return fail(error?.status || (error?.code === 'P2002' ? 409 : error?.code === 'P2034' ? 409 : 500), error?.message || 'No pudimos crear la combinación.'); }
}

export async function deleteCombination(companyId: number, combinationId: number, actorUserId: string): Promise<Result> { try { await prisma.$transaction(async (tx) => { await tx.$queryRaw`SELECT id FROM restaurant_table_combination WHERE company_id = ${companyId} AND id = ${combinationId} FOR UPDATE`; const combo = await tx.restaurantTableCombination.findFirst({ where: { id: combinationId, company_id: companyId }, include: { sessions: { where: { status: 'ACTIVE' } }, reservations: { where: { status: { in: activeStatuses } } } } }); if (!combo) throw Object.assign(new Error('No encontramos la combinación.'), { status: 404 }); if (combo.sessions.length || combo.reservations.length) throw Object.assign(new Error('No podés eliminar una combinación que está en uso.'), { status: 409 }); const changed = await tx.restaurantTableCombination.updateMany({ where: { id: combinationId, company_id: companyId, is_active: true }, data: { is_active: false } }); if (!changed.count) throw Object.assign(new Error('La combinación ya fue desactivada.'), { status: 409 }); await tx.restaurantAuditLog.create({ data: { company_id: companyId, actor_user_id: actorUserId, event: 'TABLE_COMBINATION_DEACTIVATED', combination_id: combinationId } }); }); return ok(null, 'Combinación desactivada.'); } catch (error: any) { return fail(error?.status || (error?.code === 'P2034' ? 409 : 500), error?.message || 'No pudimos desactivar la combinación.'); } }
...
export async function releaseCombinationSession(companyId: number, sessionId: number, actorUserId: string): Promise<Result> {
  try {
    const session = await prisma.$transaction(async (tx) => {
      await tx.$queryRaw`SELECT id FROM restaurant_table_combination_session WHERE company_id = ${companyId} AND id = ${sessionId} FOR UPDATE`;
      ...
      const updatedCount = await tx.restaurantTableCombinationSession.updateMany({ where: { id: sessionId, company_id: companyId, status: 'ACTIVE' }, data: { status: 'RELEASED', released_at: new Date(), released_by_user_id: actorUserId } });
      if (!updatedCount.count) throw Object.assign(new Error('La combinación ya fue separada.'), { status: 409 });
      const updated = await tx.restaurantTableCombinationSession.findFirst({ where: { id: sessionId, company_id: companyId } }); if (!updated) throw Object.assign(new Error('No encontramos la combinación activa.'), { status: 404 });
