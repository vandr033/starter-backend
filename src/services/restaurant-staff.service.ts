...
export async function updateWaiterReservationStatus(companyId: number, userId: string, reservationId: number, nextStatus: RestaurantReservationStatus): Promise<Result> {
  ...
  try {
    const updated = await prisma.$transaction(async (tx) => {
      ...
      const now = new Date(); const reservation = await tx.restaurantReservation.update({ where: { id: reservationId }, data: { status: nextStatus, seated_at: nextStatus === 'SEATED' ? now : undefined }, include: { table: true, combination: { include: { tables: true } } } });
      ...
      await tx.restaurantAuditLog.create({ data: { company_id: companyId, actor_user_id: userId, event: nextStatus === 'ARRIVED' ? 'RESERVATION_ARRIVAL_MARKED' : 'RESERVATION_SEATED', shift_id: linked.assignment.shift.id, reservation_id: reservationId, table_id: linked.tableId, metadata: { previous_status: current, status: nextStatus } } }); return reservation;
...
export async function addInternalNote(companyId: number, userId: string, input: { note: string; table_id?: number; reservation_id?: number }): Promise<Result> {
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (input.reservation_id) { const linked = await assignedReservation(companyId, userId, input.reservation_id, new Date(), tx); if (!linked) throw Object.assign(new Error('La reserva no pertenece a una mesa asignada en tu turno activo.'), { status: 403 }); assignment = linked.assignment; tableId = tableId || linked.tableId; }
