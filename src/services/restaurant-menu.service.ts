...
export async function updateCategory(companyId: number, id: number, input: any): Promise<Result> {
  ...
  try { return ok(await prisma.restaurantMenuCategory.update({ where: { id }, data: input }), 'Categoría actualizada.'); }
...
export async function deleteCategory(companyId: number, id: number): Promise<Result> {
  ...
  await prisma.restaurantMenuCategory.delete({ where: { id } }); return ok(null, 'Categoría eliminada.');
}
export async function reorderCategories(companyId: number, items: Array<{ id: number; sortOrder: number }>): Promise<Result> {
  ...
  await prisma.$transaction(items.map((item) => prisma.restaurantMenuCategory.update({ where: { id: item.id }, data: { sort_order: item.sortOrder } })));
...
export async function listItems(companyId: number, input: any): Promise<Result> {
  ...
  const [items, total] = await Promise.all([restaurantMenuRepo.listItems(companyId, filter), prisma.restaurantMenuItem.count({ where: { company_id: companyId, ...(input.categoryId ? { category_id: input.categoryId } : {}) } })]);
...
export async function updateItem(companyId: number, id: number, input: any): Promise<Result> {
  ...
  return ok(await prisma.restaurantMenuItem.update({ where: { id }, data: input }), 'Producto actualizado.');
}
export async function deleteItem(companyId: number, id: number): Promise<Result> {
  const row = await restaurantMenuRepo.findItem(companyId, id); if (!row) return fail(404, 'No encontramos el producto.');
  await prisma.restaurantMenuItem.update({ where: { id }, data: { is_active: false } }); return ok(null, 'Producto desactivado.');
}
export async function reorderItems(companyId: number, items: Array<{ id: number; sortOrder: number }>): Promise<Result> {
  ...
  await prisma.$transaction(items.map((item) => prisma.restaurantMenuItem.update({ where: { id: item.id }, data: { sort_order: item.sortOrder } })));
