...
async function validateFinancialInput(input: VisitInput, existing?: any) {
  ...
  if (subtotal === null || discount === null || tip === null || total === null || discount > subtotal) throw Object.assign(new Error('Los importes deben ser enteros no negativos y el descuento no puede superar el subtotal.'), { status: 400 });
  const paymentMethod = input.payment_method || existing?.payment_method || RestaurantVisitPaymentMethod.OTHER;
  const breakdown = input.mixed_payment_breakdown === undefined ? existing?.mixed_payment_breakdown : input.mixed_payment_breakdown;
  if (paymentMethod === RestaurantVisitPaymentMethod.MIXED) {
    ...
    const values = Object.values(breakdown) as unknown[];
    if (values.some((value) => !Number.isSafeInteger(value) || Number(value) < 0) || values.reduce<number>((sum: number, value) => sum + Number(value), 0) !== total) throw Object.assign(new Error('El desglose de pagos mixtos debe coincidir con el total pagado.'), { status: 400 });
  }
  return { guestCount, subtotal, discount, tip, total, paymentMethod, breakdown: breakdown || null };
}
...
export async function saveVisit(companyId: number, actorUserId: string, input: VisitInput, waiterOnly = false): Promise<Result> {
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      const tableId = input.table_id === undefined ? existing?.table_id ?? reservation?.table_id ?? null : input.table_id;
      ...
      if (waiterOnly) {
        ...
        const assigned = waiterUser && tableId ? await tx.restaurantShiftTableAssignment.findFirst({ where: { company_id: companyId, table_id: tableId, member: { user_id: actorUserId }, shift: { status: 'OPEN', start_at: { lte: now }, end_at: { gt: now } } } }) : null;
      ...
      const data: Prisma.RestaurantVisitUncheckedCreateInput = {
        ...
        closed_by_user_id: complete ? actorUserId : existing?.closed_by_user_id ?? null,
        closed_at: complete ? now : existing?.closed_at ?? null,
      };
      let visit: any;
      if (existing) {
      ...
      if (complete && (reservation || combinationSession)) {
        if (reservation && (reservation.status === RestaurantReservationStatus.ARRIVED || reservation.status === RestaurantReservationStatus.SEATED)) await tx.restaurantReservation.updateMany({ where: { id: reservation.id, company_id: companyId, status: { in: [RestaurantReservationStatus.ARRIVED, RestaurantReservationStatus.SEATED] } }, data: { status: RestaurantReservationStatus.COMPLETED, completed_at: now } });
        ...
        if (sessions.length) {
          await tx.restaurantTableCombinationSession.updateMany({ where: { company_id: companyId, id: { in: sessions.map((session) => session.id) } }, data: { status: 'RELEASED', released_at: now, released_by_user_id: actorUserId } });
        ...
        for (const completionTableId of completionTableIds) {
          const blocking = await tx.restaurantReservation.findFirst({ where: { company_id: companyId, status: { in: ['PENDING', 'CONFIRMED', 'ARRIVED', 'SEATED'] }, end_time: { gt: now }, OR: [{ table_id: completionTableId }, { combination: { tables: { some: { table_id: completionTableId } } } }] }, select: { id: true } });
      ...
      await tx.restaurantAuditLog.create({ data: { company_id: companyId, actor_user_id: actorUserId, shift_id: shift?.id ?? null, table_id: tableId, reservation_id: reservation?.id ?? null, event: complete ? 'VISIT_COMPLETED' : existing ? 'VISIT_FINANCIALS_UPDATED' : 'VISIT_CREATED', target_type: 'RESTAURANT_VISIT', target_id: String(visit.id), previous_values: existing ? { status: existing.status, total_paid_amount_cents: existing.total_paid_amount_cents } : Prisma.JsonNull, new_values: { status: visit.status, total_paid_amount_cents: visit.total_paid_amount_cents, payment_method: visit.payment_method } } });
      return visit;
...
export async function reopenVisit(companyId: number, id: number, actorUserId: string, reason: string): Promise<Result> {
  try {
    const visit = await prisma.$transaction(async (tx) => {
      ...
      const updated = await tx.restaurantVisit.update({ where: { id }, data: { status: RestaurantVisitStatus.REOPENED, reopened_by_user_id: actorUserId, reopened_at: new Date() } });
...
export async function financialMetrics(companyId: number, query: any): Promise<Result> {
  const company = await prisma.company.findUnique({ where: { id: companyId }, select: { timezone: true } });
  if (!company) return fail(404, 'No encontramos la empresa.');
  const where: Prisma.RestaurantVisitWhereInput = { company_id: companyId, status: { in: completedStates }, ...localBounds(company.timezone, query.dateFrom, query.dateTo) };
  const [aggregate, paymentGroups, waiterGroups, areaGroups, shiftGroups] = await prisma.$transaction([
    ...
    prisma.restaurantVisit.groupBy({ by: ['primary_waiter_user_id'], where, orderBy: { primary_waiter_user_id: 'asc' }, _count: { _all: true }, _sum: { guest_count: true, total_paid_amount_cents: true, tip_amount_cents: true } }),
    ...
    prisma.restaurantVisit.groupBy({ by: ['shift_id'], where, orderBy: { shift_id: 'asc' }, _count: { _all: true }, _sum: { guest_count: true, total_paid_amount_cents: true, tip_amount_cents: true } }),
  ]);
  ...
  return ok({ totals: { visits: totalVisits, guests, subtotal_amount_cents: aggregate._sum.subtotal_amount_cents || 0, discount_amount_cents: aggregate._sum.discount_amount_cents || 0, tip_amount_cents: aggregate._sum.tip_amount_cents || 0, total_paid_amount_cents: totalPaid, average_spend_cents: totalVisits ? Math.round(totalPaid / totalVisits) : 0, average_guest_spend_cents: guests ? Math.round(totalPaid / guests) : 0 }, by_payment_method: groups(paymentGroups, 'payment_method'), by_waiter: groups(waiterGroups, 'primary_waiter_user_id'), by_area: groups(areaGroups, 'dining_area_snapshot'), by_shift: groups(shiftGroups, 'shift_id') });
}
