...
export const depositReviewSchema = z.object({ action: z.enum(['APPROVE', 'REJECT', 'WAIVE', 'REFUND']), reason: z.string().trim().max(500).nullable().optional(), amountCents: z.number().int().positive().optional(), paymentMethod: z.enum(['CASH', 'CARD', 'QR', 'TRANSFER', 'OTHER']).nullable().optional() }).superRefine((value, ctx) => {
  if ((value.action === 'REJECT' || value.action === 'WAIVE') && !value.reason?.trim()) ctx.addIssue({ code: 'custom', path: ['reason'], message: value.action === 'WAIVE' ? 'Indicá el motivo de la exención.' : 'Indicá el motivo del rechazo.' });
});
